...
— Sends Telegram alerts with “BH vs HMM” ratio, per‐asset regime memory,
  robust JSON, and full-backtest ratio.
"""
...
for name, ticker in assets.items():
    ...
    # Persist both with the exact same scaler instance
    ...
    print(f"{ticker}: Buy & Hold → {results[ticker]['cum_market']:.4f}, "
          f"HMM Strategy → {results[ticker]['cum_hmm']:.4f}")

# ── 8. Generate Telegram Alerts ───────────────────────────────────────
LOOKBACK = 60
for name, ticker in assets.items():
    info       = results[ticker]
    model      = info["model"]
    ...
    if vcol2:
        df2["Volume_Z"] = ((df2[vcol2] - df2[vcol2].rolling(20).mean()) /
                           df2[vcol2].rolling(20).std())
    ...
    cumM, cumH = info["cum_market"], info["cum_hmm"]
    ratio_text = f"{(cumH/cumM):.2f}×" if cumM!=0 else "N/A"
    ...
    # only fire once per regime-change
    last = last_state.get(ticker)
    if last is None or curr_s != last:
        requests.post(BASE_URL, json={"chat_id":CHAT_ID, "text":msg})
        last_state[ticker] = curr_s
        STATE_FILE.write_text(json.dumps(last_state))
        print(f"{ticker}: Sent alert (Prev→Curr: {prev_s}→{curr_s}, Ratio: {ratio_text})")
    else:
        print(f"{ticker}: No regime change ({prev_s}→{curr_s}), no alert.")
